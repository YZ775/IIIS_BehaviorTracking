...
def main():
    print("select Source")
    select = input("0:from camera else: PATH\n")
    print("\n")
    ...
    out = cv2.VideoWriter('output.mp4',fourcc, 20.0, (2*frame.shape[1],frame.shape[0]))
    ###############
    ser = serial.Serial('/dev/tty.usbserial-14330', 9600,parity=serial.PARITY_NONE)
    print(ser.name)
    #マウスクリックで対象を選択
